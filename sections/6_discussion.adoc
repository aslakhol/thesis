...
They represent the complexity of the information and the presentation, respectively, which are both context-specific factors.
This is opposed to germane load, which is concerned with the subject's personal characteristics and would likely be more generalizable.
...
==== All
In pipelines with the All feature group, we combine all features and capture both context-sensitive and generalizable patterns.
Our results indicate that pipelines with the All feature group appear among both the context-sensitive and generalizable pipelines.
This might be because, in our ensemble classifier, more predictive features for the training context might be preferred.
We see that All-pipelines that are trained on the fractions_cscw tend to generalize.
This might indicate that features that perform well when predicting fractions_cscw produce more generalizable pipelines.
