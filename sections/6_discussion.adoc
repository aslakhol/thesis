...
By introducing more variablity we are optimizing for both bias and variance which is crucial for generalizability citenp:[kidzinskiGeneralizabilityMOOCModels2016].
...
The ensemble regressor will prefer features that are more predictive for the training context.
We see that All-pipelines that are trained on the fractions_cscw tend to generalize.
This might indicate that the features that perform well when predicting fractions_cscw produce more generalizable pipelines.
