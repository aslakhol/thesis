...
Since the labels of the datasets we are in different ranges we need a metric that can be compared across datasets.
NRMSE is normalized in the range from 0 to 1 where 1 is the maximum error possible for the dataset.
...
NRMSE is calculated by the following formulas.
