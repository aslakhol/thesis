...
Something about us generating fixations for EMIP from raw data


==== Data cleaning
The datasets contain missing values


==== Normalization and Outlier removal
As our subjects come from multiple contexts, the need for normalization and outlier removals is extra apparent.
...
However, as we are not allowed to share the data we received from other researchers, complete external repeatability is impossible.
