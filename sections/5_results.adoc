...
.The four best pipelines by NRMSE per dataset and whether they used dimensionality reduction or feature selection.
...
.Median NRMSE Rank and mean NRMSE for PCA and LASSO for all 1-to-1 pipelines.
...
.Median NRMSE Rank and mean NRMSE for PCA and LASSO for all 2-to-1 pipelines.
[format="csv", options="header"]
|===
...
We note that fixation duration performs well in most combinations and that spectral histograms perform well in several pipelines and ALL perform well, particularly for fractions_cscw.
...
We see that eye-tracking is the best feature group for out-of-sample testing amongst the 1-to-1 pipelines, performing slightly better than heatmaps and fixation duration.
...
.The four features with the best median NRMSE Rank and mean NRMSE for the feature groups across all 1-to-1 pipelines.
[format="csv", options="header"]
|===
...
.The four features with the best median NRMSE Rank and mean NRMSE for the feature groups across all 2-to-1 pipelines.
[format="csv", options="header"]
|===
...
Again we can see that the dataset combination is an important factor in which pipelines are exhibit context sensitivity.
...
ARMA, LHIPA, Eye Tracking, and Pupil Diameter are present in the context-sensitive pipelines but not in the more generalizable pipelines.
