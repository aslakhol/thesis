...
The baseline is equivalent to predicting the mean of the labels from the in-study dataset, then calculating the NRMSE from those predictions.
This is calculated separately for each dataset combination.
...
We do this by ranking, giving each pipeline a rank for NRMSE, then grouping on method of reducing the feature space or feature group, and taking the average.
This _mean of ranks_ gives us the results of what pipeline components perform best when testing in the same context it was trained.
...
For simplicity, since our focus is generalizability, we will only refer to the 2-to-1 pipelines in the following sections.
...
Again we can see that the dataset combination is an important factor in which pipelines exhibit context-sensitivity.
