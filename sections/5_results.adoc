...
When we aggregate the results, as seen in xref:one_to_one_features_nrmse_aggregated[] and xref:two_to_one_features_nrmse_aggregated[].
We see that eye-tracking features is the best feature grou for out-of-sample testing amongst the 1-to-1 pipelines, performing slightly better than heatmaps and fixation duration.
Piplines that include GARCH seem to outperform other pipelines but fixation duration also performs quite well.

[[one_to_one_features_nrmse_aggregated]]
.The four best pipelines by NRMSE Rank and mean NRMSE for the feature groups across all 1-to-1 pipelines.
[format="csv", options="header"]
|===
...
.The four best pipelines by NRMSE Rank and mean NRMSE for the feature groups across all 2-to-1 pipelines.
[format="csv", options="header"]
|===
...
We compare the errors from predicting on the out-of-study dataset to the out-of-sample results using FGI, which is explained in xref:feature_generalizability_index[].
To indetify the most generalizable pipelines, we need to filter out pipelines that perform poorly in out-of-sample testing.
...
It is also included in the two pipelines with the best FGI.
We note PCA performs very well for the feature groups ALL and heatmap; this might be explained by the fact that these are the largest feature groups with hundreds of thousands of values.


Many of the feature groups are represented among the more generalizable pipelines.
