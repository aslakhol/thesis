...
However, when we aggregate the results as seen in xref:one_to_one_dim_nrmse_aggregated[], and xref:two_to_one_dim_nrmse_aggregated[], we can see that Lasso performs slightly better than PCA across all pipelines.
But there is still no clear answer to which method
...
==== Generalizability
There seem to be no precise results as to if Lasso or PCA is most generalizable.
...
[[one_to_one_feature_nrmse_aggregated]]
...
[[two_to_one_feature_nrmse_aggregated]]
