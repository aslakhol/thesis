...
xref:same_training_and_testing_context[] shows how the pipelines performed in the same training and testing contexts, and aggregates the results to find out which components performed best overall.
...
Our work is focused on engineering generalizable features.
This section ourlines the results from the out-of-sample testing, which does not indicate generalizability.
